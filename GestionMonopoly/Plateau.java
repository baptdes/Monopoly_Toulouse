 ...
 * Elle contient les cases, les joueurs, les dés, les cartes, et gère le
 * déroulement du jeu.
 */
public class Plateau {
    ...
    private void creerCases() {
        ...
        // Attention : Pour numéroter les cases : la case 0 est la case de depart
        // ensuite on compte dans le sens du plateau
        ...
        cases[13] = new Propriete("Avenue Honoré Serres", 13, Pink, 140, new int[] { 10, 20, 50, 150, 450, 625, 750 },
                100);
        ...
        cases[18] = new Propriete("Rue de la République", 18, Orange, 180, new int[] { 14, 28, 70, 200, 550, 750, 950 },
                100);
        cases[19] = new Propriete("Boulevard Lazare-Carnot", 19, Orange, 200,
                new int[] { 16, 32, 80, 220, 600, 800, 1000 }, 100);
        cases[20] = new CaseParking("Parking Gratuit", 20);
        cases[21] = new Propriete("Quai de la Daurade", 21, Rouge, 220, new int[] { 18, 36, 90, 250, 700, 875, 1050 },
                150);
        cases[22] = new CaseChance("Case Chance", 22);
        cases[23] = new Propriete("Place Victor-Hugo", 23, Rouge, 220, new int[] { 18, 36, 90, 250, 700, 875, 1050 },
                150);
        cases[24] = new Propriete("Place Occitanie", 24, Rouge, 240, new int[] { 20, 40, 100, 300, 750, 925, 1100 },
                150);
        cases[25] = new Gare("Gare St-Agne", 25, 200, new int[] { 25, 50, 100, 200, 0, 0, 0 });
        cases[26] = new Propriete("Allé Jean-Jaurès", 26, Jaune, 260, new int[] { 22, 44, 110, 330, 800, 975, 1150 },
                150);
        ...
        cases[29] = new Propriete("Place Saint-Georges", 29, Jaune, 280,
                new int[] { 24, 48, 120, 360, 850, 1025, 1200 }, 150);
        ...
        cases[32] = new Propriete("Quai de Tounis", 32, Vert, 300, new int[] { 26, 52, 130, 390, 900, 1100, 1275 },
                200);
        cases[33] = new CaseCommunaute("Case Communauté", 33);
        cases[34] = new Propriete("Rue Croix Baragnon", 34, Vert, 320, new int[] { 28, 56, 150, 450, 1000, 1200, 1400 },
                200);
        cases[35] = new Gare("Téléo téléphérique", 35, 200, new int[] { 25, 50, 100, 200, 0, 0, 0 });
        cases[36] = new CaseChance("Case Chance", 36);
        cases[37] = new Propriete("Rue d'Alsace-Lorraine", 37, Bleu, 350,
                new int[] { 35, 70, 175, 500, 1100, 1300, 1500 }, 200);
        cases[38] = new CaseTaxeLuxe("Taxe de Luxe", 38, 100);
        cases[39] = new Propriete("Rue du Capitole", 39, Bleu, 400, new int[] { 50, 100, 200, 600, 1400, 1700, 2000 },
                200);
    ...
    private void creerCartesCommunaute() {
        cartesCommunaute[0] = new CarteArgent("Quartier",
                "Votre quartier veut rénover votre rue, vous devez participer à hauteur de 200 M$", 200);
    }
    ...
    /**
     * Détermine si la partie est terminée en fonction du nombre de joueurs en
     * faillite.
     * 
    ...
    public void jouerTour() {
        if (!getJoueurActif().estEnPrison()) {
            ...
            this.des.lancer();
            JFrame fenetreLancerDes = new FenetreMessageSimple(getJoueurActif().getNom() + " lance les dés ...",
            ...
            JFrame fenetreDes = new FenetreMessageSimple(getJoueurActif().getNom() + " a fait " + des.getDe1() + " et "
            ...
            System.out.println("Le joueur " + getJoueurActif().getNom() + " est tombé sur la case "
                    + getCaseJoueurActif().getNom());

            // 4 - Vérification de la case après déplacement
            fenetrePlateau.verifierCase();

            // 5 - Activation de l'effet de la case sur laquelle il se trouve
            getCaseJoueurActif().action(getJoueurActif(), this);

            // 6 - Son tour est fini
            while (!tourFini) {
                attendre(1);
            } // Attendre tant que le joueur ne signale pas qu'il a terminé
            tourFini = false;
            aLancerDes = false;
            ...
            JFrame fenetreDouble = new FenetreMessageSimple(
                    getJoueurActif().getNom() + " rejoue, elle avait fait un double !", Color.white, Color.black);
